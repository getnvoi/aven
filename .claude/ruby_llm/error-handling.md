# Error Handling | RubyLLM

Learn how to handle errors gracefully when working with AI providers

## [](#table-of-contents)Table of contents

1.  [RubyLLM Error Hierarchy](#rubyllm-error-hierarchy)
2.  [Basic Error Handling](#basic-error-handling)
3.  [Handling Specific Errors](#handling-specific-errors)
4.  [Accessing API Response Details](#accessing-api-response-details)
5.  [Error Handling During Streaming](#error-handling-during-streaming)
6.  [Handling Errors Within Tools](#handling-errors-within-tools)
7.  [Automatic Retries](#automatic-retries)
...
## [](#rubyllm-error-hierarchy)RubyLLM Error Hierarchy
...
## [](#accessing-api-response-details)Accessing API Response Details
...
When using streaming with a block, errors can occur _during_ the stream after some chunks have already been processed. The `ask` method will raise the error _after_ the block execution finishes or is interrupted by the error.
...
Your block will execute for chunks received _before_ the error. The final return value of `ask` when an error occurs during streaming might be unpredictable (often `nil`), so rely on the rescued exception for error handling.

When building [Tools](https://rubyllm.com/tools/), you need to decide how errors within the tool’s `execute` method should be handled:
