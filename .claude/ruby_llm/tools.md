...
3.  [Returning Rich Content from Tools](#returning-rich-content-from-tools)
4.  [Custom Initialization](#custom-initialization)
5.  [Using Tools in Chat](#using-tools-in-chat)
    1.  [Model Compatibility](#model-compatibility)
6.  [The Tool Execution Flow](#the-tool-execution-flow)
7.  [Monitoring Tool Calls with Callbacks](#monitoring-tool-calls-with-callbacks)
    1.  [Example: Limiting Tool Calls](#example-limiting-tool-calls)
8.  [Advanced: Halting Tool Continuation](#advanced-halting-tool-continuation)
    1.  [What halt does](#what-halt-does)
    2.  [When you might use it](#when-you-might-use-it)
    3.  [Example with sub-agents](#example-with-sub-agents)
9.  [Model Context Protocol (MCP) Support](#model-context-protocol-mcp-support)
10. [Debugging Tools](#debugging-tools)
11. [Error Handling in Tools](#error-handling-in-tools)
...
3.  **`param`:** A class method used to define each input parameter.
    ...
    - **`required:`:** (Optional, defaults to `true`) Whether the AI _must_ provide this parameter when calling the tool. Set to `false` for optional parameters and provide a default value in your `execute` method signature.
...
[](#returning-rich-content-from-tools)Returning Rich Content from Tools
-----------------------------------------------------------------------
...
def execute(query:) # Generate analysis and create visualization
...
def generate*chart(query) # Your chart generation logic
"/tmp/chart*#{Time.now.to_i}.png"
end
end
...
def execute(query:, limit: 5) # Search in @database
...
3.  **Tool Call Request:** The model responds _not_ with text, but with a special message indicating a tool call, including the tool name (`weather_lookup`) and arguments (`{ latitude: 52.52, longitude: 13.40 }`).
...
.with_tool(Weather)
.on_tool_call do |tool_call| # Called when the AI decides to use a tool
puts "Calling tool: #{tool_call.name}"
puts "Arguments: #{tool_call.arguments}"
end
.on_tool_result do |result| # Called after the tool returns its result
...
### [](#example-limiting-tool-calls)Example: Limiting Tool Calls
...
### [](#when-you-might-use-it)When you might use it
...
### [](#example-with-sub-agents)Example with sub-agents
...
See the [Error Handling Guide](about:/error-handling/#debugging) for more on debugging.
...
See the [Error Handling Guide](about:/error-handling/#handling-errors-within-tools) for more discussion.
...
*   [Streaming Responses](https://rubyllm.com/streaming/) (See how tools interact with streaming)
*   [Rails Integration](https://rubyllm.com/rails/) (Persisting tool calls and results)
