# Stream Responses | RubyLLM

Learn how to display AI responses in real-time as they’re generated

## [](#table-of-contents)Table of contents

1.  [Basic Streaming](#basic-streaming)
2.  [Understanding Chunks](#understanding-chunks)
3.  [Accumulated Response](#accumulated-response)
4.  [Web Application Integration](#web-application-integration)
    1.  [Rails with Turbo Streams](#rails-with-turbo-streams)
    2.  [Sinatra with Server-Sent Events (SSE)](#sinatra-with-server-sent-events-sse)
5.  [Error Handling During Streaming](#error-handling-during-streaming)
6.  [Streaming with Tools](#streaming-with-tools)
...
- `chunk.tool_calls`: A hash containing partial or complete tool call information if the model is invoking a [Tool](https://rubyllm.com/tools/). The arguments might be streamed incrementally.
- `chunk.input_tokens`: Total input tokens for the request (often `nil` until the final chunk).
- `chunk.output_tokens`: Cumulative output tokens _up to this chunk_ (behavior varies by provider, often only accurate in the final chunk).
...
## [](#accumulated-response)Accumulated Response

Even when you provide a block for streaming, the `ask` method _still_ returns the complete, final `RubyLLM::Message` object once the entire response (including any tool interactions) is finished.
...
### [](#rails-with-turbo-streams)Rails with Turbo Streams
...
See the [Rails Integration Guide](about:/rails/#streaming-responses-with-hotwireturbo) for more detailed examples.

### [](#sinatra-with-server-sent-events-sse)Sinatra with Server-Sent Events (SSE)
...
## [](#error-handling-during-streaming)Error Handling During Streaming
...
Refer to the [Error Handling Guide](https://rubyllm.com/error-handling/) for details on specific error types.

When a chat interaction involves [Tools](https://rubyllm.com/tools/), the streaming behavior has distinct phases:

1.  **Initial Response Stream:** Chunks are yielded as the model generates text _up to_ the point where it decides to call a tool.
...
```
chat = RubyLLM.chat(model: 'gpt-4.1').with_tool(Weather) # Assumes Weather tool is defined
